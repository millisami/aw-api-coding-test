...
RSpec.describe "V1::Auth", type: :request do
  describe "POST /auth/signin" do
    ...
    it "authenticates and return a token" do
      post "/v1/auth/signin", params: {auth: { email: user.email, password: 'sekret' }}
    ...
    it "returns error when email does not exist" do
      post "/v1/auth/signin", params: {auth: { email: 'noemail@host.com', password: 'sekret' }}
    ...
    it "returns error when password does not match" do
      post "/v1/auth/signin", params: {auth: { email: user.email, password: 'wrong' }}
